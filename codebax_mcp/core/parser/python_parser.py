...
class PythonParser(BaseParser):
    ...
    def _extract_symbols(self, node: ast.AST, symbols: List[Symbol], file_path: str, content: str, parent_id: Optional[str] = None) -> None:
    ...
    def _get_range(self, node: ast.AST, lines: List[str]) -> Range:
        ...
        end_col_offset = getattr(node, 'end_col_offset', len(lines[node.lineno - 1]) if node.lineno <= len(lines) else 0)
