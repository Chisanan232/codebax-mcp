...
def create_or_update_for_symbol(input: CreateOrUpdateForSymbolInput) -> CreateOrUpdateForSymbolOutput:
    ...
    if input.intent not in ["add_or_update", "delete"]:
        return CreateOrUpdateForSymbolOutput(
            status="failed", test_files_modified=[], error=f"Invalid intent: {input.intent}"
        )
    ...
    if not test_file:
        return CreateOrUpdateForSymbolOutput(
            status="failed", test_files_modified=[], error="Could not find or create test file"
        )
    ...
    return CreateOrUpdateForSymbolOutput(status="ok", test_files_modified=test_files_modified)
