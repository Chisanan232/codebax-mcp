...
from codebax_mcp.mcp_server.models.output import ExistingTest, LocateForSourceOutput
...
def locate_for_source(input: LocateForSourceInput) -> LocateForSourceOutput:
    ...
    return LocateForSourceOutput(
        source_path=input.source_path, existing_tests=existing_tests, suggested_new_test_path=suggested_path
    )
...
def _suggest_test_path(source_path: str, workspace_root: str) -> str | None:
...
def _detect_test_framework(test_file: str) -> str | None:
