...
from codebax_mcp.mcp_server.models.output import SubprojectsOutput, SubprojectInfo
    ...
    for item in os.listdir(workspace_root):
        ...
        if os.path.isdir(path) and not item.startswith('.'):
            if os.path.exists(os.path.join(path, "pyproject.toml")):
                subprojects.append(SubprojectInfo(
                    name=item,
                    type="python",
                    path=item,
...
def _detect_node_subprojects(workspace_root: str) -> List[SubprojectInfo]:
    ...
    try:
        with open(os.path.join(workspace_root, "package.json"), 'r') as f:
            pkg = json.load(f)
...
def _detect_gradle_subprojects(workspace_root: str) -> List[SubprojectInfo]:
    ...
    try:
        with open(os.path.join(workspace_root, "settings.gradle"), 'r') as f:
            content = f.read()
