...
def find_usages(input: FindUsagesInput) -> FindUsagesOutput:
    ...
    try:
        ...
        if not index_data:
            return FindUsagesOutput(symbol_id=input.symbol_id, usages=[], total=0, notes="Index not found")

        # Search for usages (simplified - just find matching names)
        symbol_data = index_data.get("symbol_definitions", {}).get(input.symbol_id)
        if not symbol_data:
            return FindUsagesOutput(symbol_id=input.symbol_id, usages=[], total=0)
        ...
        return FindUsagesOutput(symbol_id=input.symbol_id, usages=usages, total=len(usages))

    except Exception as e:
        return FindUsagesOutput(symbol_id=input.symbol_id, usages=[], total=0, notes=f"Error: {e!s}")
