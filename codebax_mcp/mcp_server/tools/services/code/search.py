...
from codebax_mcp.mcp_server.models.output import SearchResult, SemanticSearchOutput
...
def semantic_search(input: SemanticSearchInput) -> SemanticSearchOutput:
    ...
    try:
        ...
        if not index_data:
            return SemanticSearchOutput(query=input.query, results=[], total=0, notes="Index not found")
        ...
        return SemanticSearchOutput(query=input.query, results=[], total=0, notes=f"Error: {e!s}")
