...
from codebax_mcp.mcp_server.tools.services.code.python_tools import (
    analyze_python_patch_calls as _analyze_python_patch_calls,
)
...
async def code_index(workspace_root: str, full: bool = False, paths: list[str] | None = None) -> IndexCodebaseOutput:
...
async def code_identify_symbol(
    ...
    """
    return _identify_symbol(
        IdentifySymbolInput(file=file, language=language, line=line, column=column, workspace_root=workspace_root)
    )
...
async def code_find_usages(
    ...
    return _find_usages(
        FindUsagesInput(
            symbol_id=symbol_id, include_tests=include_tests, max_results=max_results, workspace_root=workspace_root
        )
    )
...
async def code_get_call_graph(
    ...
    return _get_call_graph(
        GetCallGraphInput(symbol_id=symbol_id, direction=direction, depth=depth, workspace_root=workspace_root)
    )
...
async def code_rename_symbol(
    ...
    """
    return _rename_symbol(
        RenameSymbolInput(symbol_id=symbol_id, new_name=new_name, dry_run=dry_run, workspace_root=workspace_root)
    )
...
async def code_extract_function(
    ...
    return _extract_function(
        ExtractFunctionInput(
            file=file, language=language, range=range, new_name=new_name, dry_run=dry_run, workspace_root=workspace_root
        )
    )
