...
class IndexCodebaseInput(BaseModel):
    ...
    full: bool = Field(default=False, description="If True, rebuild entire index. If False, update only changed files")
    paths: list[str] | None = Field(default=None, description="Optional list of specific file paths to index")
...
class GetDefinitionInput(BaseModel):
    ...
    symbol_id: str = Field(..., description="Unique identifier of the symbol")
    workspace_root: str = Field(default=".", description="Path to the workspace root directory")
...
class FindUsagesInput(BaseModel):
    ...
    symbol_id: str = Field(..., description="Unique identifier of the symbol")
    include_tests: bool = Field(default=False, description="If True, include usages in test files")
...
class GetCallGraphInput(BaseModel):
    ...
    symbol_id: str = Field(..., description="Unique identifier of the symbol")
    direction: str = Field(
        default="both", description="Direction to trace - 'incoming' (callers), 'outgoing' (callees), or 'both'"
    )
...
class RenameSymbolInput(BaseModel):
    ...
    dry_run: bool = Field(default=True, description="If True, preview changes without applying")
    workspace_root: str = Field(default=".", description="Path to the workspace root directory")
...
class ExtractFunctionInput(BaseModel):
    ...
    file: str = Field(..., description="Path to the file (relative to workspace_root)")
    language: str = Field(..., description="Programming language (python, typescript, java, etc.)")
    range: CodeRange = Field(..., description="Code range to extract")
    new_name: str = Field(..., description="Name for the extracted function")
    dry_run: bool = Field(default=True, description="If True, only return proposed changes without applying")
    workspace_root: str = Field(default=".", description="Path to the workspace root directory")
...
class SemanticSearchInput(BaseModel):
    ...
    filter_language: str | None = Field(
        default=None, description="Optional language filter (python, typescript, java, etc.)"
    )
    filter_path_prefix: str | None = Field(default=None, description="Optional path prefix filter")
    workspace_root: str = Field(default=".", description="Path to the workspace root directory")
