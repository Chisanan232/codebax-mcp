...
class GetSymbolInfoInput(BaseModel):
    ...
    context_range: Optional[ContextRange] = Field(
        default=None,
        description="Optional context range for disambiguation"
    )
...
class FindSimilarUsagesInput(BaseModel):
    ...
    symbol: str = Field(
        ...,
        description="Symbol name to search for similar symbols"
    )
    language: str = Field(
        default="python",
        description="Programming language"
...
class InferExpressionInput(BaseModel):
    ...
    context_range: Optional[ContextRange] = Field(
        default=None,
        description="Optional context range for analysis"
    )
    workspace_root: str = Field(
        default=".",
        description="Path to the workspace root directory"
...
class ValidateChangesInput(BaseModel):
    ...
    dry_run: bool = Field(
        default=True,
        description="If True, don't apply changes"
    )
    workspace_root: str = Field(
        default=".",
        description="Path to the workspace root directory"
...
class SuggestFixInput(BaseModel):
    ...
    column: int = Field(
        ...,
        description="Column number of the error (0-indexed)"
    )
    error_message: str = Field(
        ...,
        description="Error message from type checker"
    )
    language: str = Field(
        default="python",
        description="Programming language"
