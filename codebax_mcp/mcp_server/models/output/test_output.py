...
class ExistingTest(BaseModel):
    ...
    framework: str | None = Field(None, description="Test framework (pytest, unittest, nose)")
...
class LocateForSourceOutput(BaseModel):
    ...
    existing_tests: list[ExistingTest] = Field(default_factory=list, description="List of found test files")
    suggested_new_test_path: str | None = Field(None, description="Suggested path for new test if none found")
    notes: str | None = Field(None, description="Additional information")
...
class CreateOrUpdateForSymbolOutput(BaseModel):
    ...
    test_files_modified: list[TestFileModification] = Field(
        default_factory=list, description="List of modified test files"
    )
