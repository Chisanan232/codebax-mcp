...
class FindUsagesOutput(BaseModel):
    ...
    symbol_id: str = Field(..., description="The symbol ID")
    usages: list[UsageLocation] = Field(default_factory=list, description="List of usage locations")
...
class GetCallGraphOutput(BaseModel):
    ...
    nodes: list[CallGraphNode] = Field(default_factory=list, description="Nodes in the call graph")
    edges: list[CallGraphEdge] = Field(default_factory=list, description="Edges in the call graph")
    notes: str | None = Field(None, description="Additional information")
...
class RenameSymbolOutput(BaseModel):
    ...
    changes: list[CodeChange] = Field(default_factory=list, description="List of proposed changes")
    error: str | None = Field(None, description="Error message if failed")
...
class SemanticSearchOutput(BaseModel):
    ...
    results: list[SearchResult] = Field(default_factory=list, description="List of matching symbols with scores")
    total: int = Field(0, description="Total number of results")
    notes: str | None = Field(None, description="Information about search method")
...
class AnalyzePythonPatchCallsOutput(BaseModel):
    ...
    patches: list[PatchCall] = Field(
        default_factory=list, description="List of patch calls with target, location, and confidence"
    )
    total: int = Field(0, description="Total number of patches found")
    by_target: dict[str, list[str]] = Field(default_factory=dict, description="Patches grouped by target string")
    notes: str | None = Field(None, description="Additional information")
