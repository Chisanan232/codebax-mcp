...
class TestTypeEndpoints:
    ...
    async def test_get_symbol_info_endpoint(self, mock_service):
        ...
        from codebax_mcp.mcp_server.models.output import (
            DefinedIn, SignatureInfo, ParameterInfo, ReturnType, NormalizedType
        )
        ...
        mock_service.return_value = GetSymbolInfoOutput(
            ...
            signature=SignatureInfo(
                parameters=[],
                return_type=ReturnType(raw=None, normalized=NormalizedType(kind="unknown", types=[]))
            ),
            confidence=0.8,
        )
    ...
    async def test_get_expected_at_position_endpoint(self, mock_service):
        ...
        from codebax_mcp.mcp_server.models.output import Position, ExpectedType, NormalizedType
