...
from codebax_mcp.mcp_server.tools.services.code.navigation import find_usages, get_definition, identify_symbol
...
""")
        ...
        # Index the project
        input_data = IndexCodebaseInput(workspace_root=str(tmp_path), full=True)
    ...
    def test_identify_and_get_definition_workflow(self, indexed_project):
        ...
        if hasattr(identify_result, "symbol_id") and identify_result.symbol_id:
            definition_input = GetDefinitionInput(
                symbol_id=identify_result.symbol_id, workspace_root=str(indexed_project)
            )
    ...
    def test_find_usages_workflow(self, indexed_project):
        ...
        if data and "symbol_definitions" in data:
            ...
            calculator_symbols = [
                s
                for s in data["symbol_definitions"].values()
            ...
            if calculator_symbols:
                ...
                # Find usages
                usages_input = FindUsagesInput(symbol_id=symbol_id, workspace_root=str(indexed_project))
    ...
    def test_navigate_to_method_definition_workflow(self, indexed_project):
        ...
        if data and "symbol_definitions" in data:
            ...
            if add_symbols:
                ...
                # Get definition
                definition_input = GetDefinitionInput(symbol_id=symbol_id, workspace_root=str(indexed_project))
    ...
    def test_cross_file_navigation_workflow(self, indexed_project):
        ...
        if data and "symbol_definitions" in data:
            ...
            calculator_file_symbols = [
                s for s in data["symbol_definitions"].values() if "calculator.py" in s.get("file", "")
            ]

            main_file_symbols = [s for s in data["symbol_definitions"].values() if "main.py" in s.get("file", "")]
